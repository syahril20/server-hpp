...
func CreateHpiResultsHandler(c *gin.Context) {
	...
	if req.IdHpi == primitive.NilObjectID {
		c.JSON(http.StatusBadRequest, dto.ErrorResult{
			Code:    http.StatusBadRequest,
	...
	idHpi, err := primitive.ObjectIDFromHex(req.IdHpi.Hex())
	if err != nil {
		c.JSON(http.StatusBadRequest, dto.ErrorResult{
			Code:    http.StatusBadRequest,
	...
	hpiData, err := repositories.GetHPIById(context.Background(), idHpi)
	if err != nil {
		c.JSON(http.StatusInternalServerError, dto.ErrorResult{
			Code:    http.StatusInternalServerError,
			Message: err.Error()})
